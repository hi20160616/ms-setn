...
import (
	...
	"github.com/hi20160616/ms-setn/configs"
...
func NewArticle() *Article {
	return &Article{
		WebsiteDomain: configs.Data.MS["setn"].Domain,
		WebsiteTitle:  configs.Data.MS["setn"].Title,
		WebsiteId:     fmt.Sprintf("%x", md5.Sum([]byte(configs.Data.MS["setn"].Domain))),
	}
}
...
var timeout = func() time.Duration {
	t, err := time.ParseDuration(configs.Data.MS["setn"].Timeout)
	if err != nil {
		log.Printf("[%s] timeout init error: %v",
			configs.Data.MS["setn"].Title, err)
...
func (a *Article) fetchUpdateTime() (*timestamppb.Timestamp, error) {
	if a.raw == nil {
		return nil, errors.Errorf("[%s] fetchUpdateTime: raw is nil: %s",
			configs.Data.MS["setn"].Title, a.U.String())
	}
